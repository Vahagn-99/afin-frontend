<template>
  ...
  <ClosedBillFilter v-if="isOpenFilter" @close-filter="closeFilter" @filter="filter" @clear="clear"/>
</template>
<script setup>
...
const closedBillStore=useClosedBillStore()
...
function changeCount(p){
  ...
  setTimeout(async () => {
    await closedBillStore.getClosedBills()
