<template>
  ...
  <RatingAllFilter v-if="isOpenFilter" @close-filter="closeFilter" @filter="filter" @clear="clear"/>
</template>
...
import {useAllRatingStore} from "@/stores/rating_all.js";
import RatingAllFilter from "@/modals/filter/RatingAllFilter.vue";
import Lucide from "@/base-components/Lucide/index.ts";
import Button from "@/base-components/Button/index.ts";

const isLoading=ref(false)
const isOpenFilter=ref(false)
const ratingAllStore=useAllRatingStore()
const {query,page,per_page,meta,ratings}=storeToRefs(ratingAllStore)
...
function changeCount(p) {
  ...
  setTimeout(async () => {
    await ratingAllStore.getAllRatings()
